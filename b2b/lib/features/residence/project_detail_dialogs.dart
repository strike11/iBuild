...
/// Result of the lead editor dialog: an updated score + tag list.
class _LeadEditResult {
  const _LeadEditResult({required this.score, required this.tags});

  final String? score;
  final List<String> tags;
}

/// Editor for a lead's score (hot/warm/cold) and free-form tags.
class _LeadEditorDialog extends StatefulWidget {
  const _LeadEditorDialog({required this.lead});
...
class _LeadEditorDialogState extends State<_LeadEditorDialog> {
  ...
  void initState() {
    ...
    _tags = TextEditingController(
      text: (widget.lead['tags'] as List? ?? []).join(', '),
    );
  }
  ...
  Widget build(BuildContext context) {
    ...
    return AlertDialog(
      ...
      title: Text(l10n.projectLeadEditorTitle),
      content: Column(
        mainAxisSize: MainAxisSize.min,
        crossAxisAlignment: CrossAxisAlignment.start,
        children: [
          ...
          TextField(
            ...
            decoration: InputDecoration(
              labelText: l10n.projectTagsLabel,
              hintText: l10n.projectTagsHint,
            ),
          ),
        ],
      ),
        ...
        PillButton(
          ...
          onPressed: () {
            final tags = _tags.text
                ...
                .toList();
            Navigator.pop(context, _LeadEditResult(score: _score, tags: tags));
          },
        ),
      ],
    );
