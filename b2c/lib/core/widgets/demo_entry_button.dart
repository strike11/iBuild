...
/// Awards / reviewer entry — read-only demo session via `POST /v1/auth/demo`.
...
class _DemoEntryButtonState extends ConsumerState<DemoEntryButton> {
  ...
  Future<void> _enterDemo() async {
    ...
    try {
      ...
      context.go(
        target != null && target.startsWith('/') ? target : '/home',
      );
    } catch (_) {
      if (!mounted) return;
...
/// Spacing helper below primary auth CTAs.
class DemoEntrySection extends ConsumerWidget {
  const DemoEntrySection({
    super.key,
    this.expand = false,
    this.redirect,
  });
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    return Column(
      ...
      children: [
        ...
        DemoEntryButton(expand: expand, redirect: redirect),
      ],
    );
  }
}
