...
/// Real-time events pushed by the API (plan section 8, WebSocket events).
enum WsEventType {
  unitStatusChanged,
  unitPriceChanged,
  leadCreated,
...
class WsClient {
  ...
  WsClient(
    ...
    String? Function()? tokenProvider,
       ...
       _tokenProvider = tokenProvider ?? (() => AuthTokenCache.accessToken);
  ...
  /// Idempotent: returns the single shared event stream, opening the socket on
  /// the first call when a token is available. Safe to call from multiple
  /// providers — later callers just receive the same broadcast stream.
  ///
  /// Does **not** bypass an in-flight backoff timer (that was flooding `/v1/ws`
  /// with unauthenticated 401s every second).
...
final wsClientProvider = Provider<WsClient>((ref) {
  final client = WsClient(Env.wsUrl, enabled: !Env.useMockData);
  final unsubscribe = AuthTokenCache.addListener((_) => client.onAuthChanged());
